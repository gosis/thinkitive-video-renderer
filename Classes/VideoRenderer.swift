...
public class VideoRenderer {
    ...
    /// Export given image or video PHAsset for given resolution and adding provided OverlayRenderable instances
    ///
    /// For image PHAssets the duration is used to create a video
    /// For video PHAssets duration is unused
    /// Max composition duration 60 seconds
    ///
    /// Existing frames of OverlayRenderable will be used to add the overlay to the video
    ///
    /// Example: if  1920x1080 resolution is provided and a single OverlayRenderable  has a rect CGrect(x: 0, y: 0, width: 1920, height: 1080)
    ///     then the overlay will cover the entire video
    ///
    /// Completion block provides a url pointing to the exported video file, if no url is returned look at error.localizedDescription for error string
    /// - Parameters:
    ///   - phAsset: PHAsset picked from photos, must be of either video or image type
    ///   - duration: explicit duration in seconds of video in case of image asset
    ///   - resolution: resolution of video or image
    ///   - overlays: array of OverlayRenderable
    ///   - completion: block to be called when video rendering is complete and video is saved in provided url
    ...
    /// Helper class that saves the file  at provided url in Photo library
    ///
    /// Used to save the rendered video in photos with url returned in completion block of exportVideoAsset()
    /// - Parameters:
    ///   - url: file system url of the video file
    ///   - completion: block to be called from the main thread when the saving is complete
...
private extension VideoRenderer {
    
    private class func videoAssetFromImage(image: UIImage,
                                   length: TimeInterval,
    ...
    private class func pixelBuffer(fromImage image: CGImage, size: CGSize) throws -> CVPixelBuffer? {
